...
function generateMessageStep(node: WorkflowNode, indent: string): string {
	...
	if (channel === "email") {
		...
		code += `${indent}\t\tto: event.payload.recipientEmail as string,\n`;
		if (templateName) {
		...
		code += `${indent}\t\tto: event.payload.recipientPhone as string,\n`;
		if (body) {
...
function generateFlagChangeStep(node: WorkflowNode, indent: string): string {
	...
	if (flagChanges.length > 0) {
		const changes = flagChanges
			...
			.join(", ");
		code += `${indent}\tconst svcResponse = await this.env.WORKFLOW_SVC.fetch(\n`;
		code += `${indent}\t\t\`http://workflow-svc/internal/workflows/\${this.env.WORKFLOW_ID}/flags/batch-update-state\`,\n`;
		code += `${indent}\t\t{\n`;
		code += `${indent}\t\t\tmethod: "POST",\n`;
		code += `${indent}\t\t\theaders: { "Content-Type": "application/json" },\n`;
		code += `${indent}\t\t\tbody: JSON.stringify({\n`;
		code += `${indent}\t\t\t\tchanges: [${changes}],\n`;
		code += `${indent}\t\t\t\tinstanceId: event.instanceId,\n`;
		code += `${indent}\t\t\t}),\n`;
		code += `${indent}\t\t},\n`;
		code += `${indent}\t);\n`;
		code += `${indent}\tif (!svcResponse.ok) {\n`;
		code += `${indent}\t\tthrow new Error(\`Flag update failed: \${svcResponse.status}\`);\n`;
		code += `${indent}\t}\n`;
	} else {
