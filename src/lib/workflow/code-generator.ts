...
function generateNLSStep(
	...
	if (functionId === "prequalification") {
		...
		// createLoan: fall back to a cases-svc-allocated loan number when the
		// node doesn't map one explicitly. `allocateLoanNumber` is idempotent
		// per case (retries of this step return the same value) and persists
		// the loan's own `YY-MM-DD-<cifno>-<consecutive>` number — separate
		// from and no longer derived from the case number.
		if (
			functionId === "createLoan" &&
			!fields.some((f) => f.fieldId === "loanNumber" && f.value)
		) {
			code += `${indent}\t_nlsBody["loanNumber"] = await this.env.CASES_SVC.allocateLoanNumber(event.payload.caseId as string);\n`;
		}
		if (
			functionId === "createLoan" &&
...
function emitPromotionOutputAssignment(
	...
	code += `${indent}\t\tnetLoanAmount: Number((${rawVar} as { payload: { netLoanAmount?: number } }).payload?.netLoanAmount ?? 0),\n`;
...
export function generateWorkflowCode(
	...
	const hasCreateLoanNodes = nodes.some(
		(n) =>
			n.type === "NLS" &&
			((n.config as NLSNodeConfig | undefined)?.functionId ?? "createLoan") ===
				"createLoan",
	);
	...
	if (hasCreateLoanNodes) {
		code += `\t\tallocateLoanNumber: (caseId: string) => Promise<string>;\n`;
	}
